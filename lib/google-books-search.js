...
var search = function(query, options, callback) {
    ...
    if (!query && !options.fields) {
        callback(new Error("Query or fields are required"));
        return;
    }
    ...
    for(field in options.fields){
        optionsString.push(fields[field] + options.fields[field])
    }
    optionsString = optionsString.join('+');
    if(query){
        query = query + '+' + optionsString
    } else {
        query = optionsString
    }
    ...
    if (options.key) {
        remainingOptions.key = options.key;
    }

    if (options.lang) {
        remainingOptions.langRestrict = options.lang;
    }


    var uri = baseUrl + '?q=' + encodeURIComponent(query) + '&' + querystring.stringify(remainingOptions);
